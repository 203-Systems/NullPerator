...
struct WavReadLimits {
  std::uint32_t minimumSampleRate = 1;
  std::uint32_t maximumSampleRate = std::numeric_limits<std::uint32_t>::max();
};
...
public:
  ...
  static etl::expected<WavHeaderInfo, WAVEFILE_ERROR>
  ReadHeader(I_File *file, WavReadLimits limits = {});
};
