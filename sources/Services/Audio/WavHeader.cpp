...
WavHeaderWriter::ReadHeader(I_File *file, WavReadLimits limits) {
  if (!file) {
  ...
  if (info.sampleRate < limits.minimumSampleRate ||
      info.sampleRate > limits.maximumSampleRate) {
...
bool WavHeaderWriter::UpdateFileSize(I_File *file, uint32_t sampleCount,
  ...
  if (finalPosition < 0 || static_cast<uint64_t>(finalPosition) >
                               std::numeric_limits<uint32_t>::max()) {
    Trace::Error("WAVHEADER: invalid final file position (%ld)", finalPosition);
    return false;
  }
  ...
  const uint64_t dataSize =
      static_cast<uint64_t>(sampleCount) * channels * bytesPerSample;
  if (logicalFileSize < 8U || dataSize > std::numeric_limits<uint32_t>::max() ||
    ...
    Trace::Error("WAVHEADER: inconsistent final size (%u) and data size (%llu)",
                 logicalFileSize, static_cast<unsigned long long>(dataSize));
    return false;
  }
