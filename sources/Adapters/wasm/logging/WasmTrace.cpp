...
#ifdef __EMSCRIPTEN__
  const double value = emscripten_get_now() * 1000.0;
...
std::atomic<WasmTrace *> WasmTrace::instance_{nullptr};

WasmTrace::WasmTrace(NowFunction now) noexcept : now_(now == nullptr ? DefaultNow : now) {
  instance_.store(this, std::memory_order_release);
}

WasmTrace::~WasmTrace() {
  FlushLine();
  WasmTrace *expected = this;
  (void)instance_.compare_exchange_strong(expected, nullptr, std::memory_order_acq_rel);
}

WasmTrace *WasmTrace::Instance() noexcept { return instance_.load(std::memory_order_acquire); }
std::uint64_t WasmTrace::Now() const noexcept { return now_ == nullptr ? 0 : now_(); }

void WasmTrace::PutChar(int character) noexcept {
  if (character == '\r') return;
  if (character == '\n') { PublishLine(); return; }
  if (lineLength_ + 1 < line_.size()) line_[lineLength_++] = static_cast<char>(character);
  else lineTruncated_ = true;
}

void WasmTrace::FlushLine() noexcept { if (lineLength_ != 0 || lineTruncated_) PublishLine(); }

void WasmTrace::PublishLine() noexcept {
  if (lineLength_ == 0 && !lineTruncated_) return;
  ...
  if (StartsWith(cursor, remaining, "[*ERROR*]")) {
    record.severity = WasmLogSeverity::Error;
    cursor += 9; remaining -= 9;
  }
  while (remaining && *cursor == ' ') { ++cursor; --remaining; }
  if (remaining >= 3 && *cursor == '[') {
    const char *close = static_cast<const char *>(std::memchr(cursor + 1, ']', remaining - 1));
    if (close != nullptr) {
      category = cursor + 1;
  ...
  while (remaining && *cursor == ' ') { ++cursor; --remaining; }
  if (record.severity != WasmLogSeverity::Error) {
    if (categoryLength == 3 && std::memcmp(category, "-D-", 3) == 0)
      record.severity = WasmLogSeverity::Debug;
    else if ((categoryLength == 4 && std::memcmp(category, "WARN", 4) == 0) ||
             (categoryLength == 7 && std::memcmp(category, "WARNING", 7) == 0))
      record.severity = WasmLogSeverity::Warn;
  }
  record.categoryLength = static_cast<std::uint8_t>(CopyText(record.category, category, categoryLength));
  if (record.categoryLength < categoryLength) record.truncated = true;
  record.threadLength = static_cast<std::uint8_t>(CopyText(record.thread, "application", 11));
  record.messageLength = static_cast<std::uint16_t>(CopyText(record.message, cursor, remaining));
  if (record.messageLength < remaining) record.truncated = true;
  (void)Push(record);
  lineLength_ = 0;
  lineTruncated_ = false;
}

bool WasmTrace::Push(const WasmLogRecord &record) noexcept {
  ...
  if (write - read >= QueueCapacity) { SaturatingIncrement(dropped_); return false; }
  queue_[write & (QueueCapacity - 1)] = record;
  writePosition_.store(write + 1, std::memory_order_release);
  return true;
}

bool WasmTrace::TryPop(WasmLogRecord &record) noexcept {
  const std::uint64_t read = readPosition_.load(std::memory_order_relaxed);
  if (read == writePosition_.load(std::memory_order_acquire)) return false;
  record = queue_[read & (QueueCapacity - 1)];
  readPosition_.store(read + 1, std::memory_order_release);
  return true;
}

std::uint64_t WasmTrace::Dropped() const noexcept { return dropped_.load(std::memory_order_relaxed); }

std::uintptr_t WasmTrace::Drain() noexcept {
  std::array<WasmLogRecord, DrainCapacity> records{};
  std::uint32_t count = 0;
  while (count < records.size() && TryPop(records[count])) ++count;
  ...
  Store<std::uint64_t>(drain_.data() + 24, 0); // reserved; producer state is not read cross-thread
  for (std::uint32_t index = 0; index < count; ++index) {
    const WasmLogRecord &source = records[index];
...
extern "C" WASM_LOG_EXPORT std::uintptr_t PicoTracker_Wasm_LogDrain() {
  WasmTrace *trace = WasmTrace::Instance();
