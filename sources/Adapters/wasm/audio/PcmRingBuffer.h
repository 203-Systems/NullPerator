...
#ifndef PICOTRACKER_WASM_PCM_RING_BUFFER_H
#define PICOTRACKER_WASM_PCM_RING_BUFFER_H
...
public:
  ...
  [[nodiscard]] std::size_t Write(std::span<const StereoI16> input) noexcept {
    ...
    const std::size_t accepted = input.size() < available ? input.size() : available;
  ...
  [[nodiscard]]
  std::size_t WriteInterleaved(std::span<const short> samples) noexcept {
  ...
  [[nodiscard]] std::size_t Read(std::span<StereoF32> output) noexcept {
    ...
    const std::size_t copied = output.size() < available ? output.size() : available;
