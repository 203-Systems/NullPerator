...
#ifndef PICOTRACKER_WASM_TRACE_RING_BUFFER_H
#define PICOTRACKER_WASM_TRACE_RING_BUFFER_H
...
template <std::size_t CapacityRecords> class TraceRingBuffer {
  static_assert(CapacityRecords != 0 && (CapacityRecords & (CapacityRecords - 1)) == 0);
...
public:
  ...
  [[nodiscard]] bool TryPop(TraceRecord &record) noexcept {
    ...
    for (;;) {
      ...
      const std::int64_t difference =
          static_cast<std::int64_t>(publication) -
          static_cast<std::int64_t>(position + 1U);
      if (difference == 0) {
        if (readPosition_.compare_exchange_weak(
                position, position + 1U, std::memory_order_relaxed,
                std::memory_order_relaxed)) {
          break;
        }
  ...
  [[nodiscard]] std::uint64_t Written() const noexcept { return writePosition_.load(std::memory_order_acquire); }
