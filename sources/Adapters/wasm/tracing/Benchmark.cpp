...
void RenderBlock(std::uint32_t block, std::array<std::uint32_t, 8> &phases,
  ...
  for (std::uint32_t frame = 0; frame < 128; ++frame) {
    ...
    for (std::uint32_t channel = 0; channel < phases.size(); ++channel) {
      ...
      const std::int32_t triangle = static_cast<std::int32_t>(
          ((phases[channel] >> 16) ^ ((phases[channel] & 0x80000000U) ? 0xffffU : 0U)) & 0xffffU) - 32768;
    ...
    const std::uint16_t right16 = static_cast<std::uint16_t>(Clamp16(right / 20));
...
WasmBenchmarkResult WasmBenchmark::Run(WasmBenchmarkConfig config, NowFunction now) noexcept {
  WasmBenchmarkResult result{};
  if (config.version != WasmBenchmarkConfig::Version) return result;
  ...
  if (now == nullptr) now = DefaultNow;
  result.iterations = config.iterations;
  result.warmupIterations = config.warmupIterations;
  result.sampleCount = config.iterations;
  std::array<std::uint32_t, 8> phases{0x12345678U, 0x23456789U, 0x3456789aU, 0x456789abU,
  ...
  for (std::uint32_t block = 0; block < config.iterations; ++block) {
    ...
    WasmProfiler::Emit(WasmTraceCategory::Benchmark, WasmTraceName::BenchmarkBlock,
                       WasmTracePhase::Begin, block, WasmTraceThread::Browser, generation);
    RenderBlock(block, phases, hash);
    WasmProfiler::Emit(WasmTraceCategory::Benchmark, WasmTraceName::BenchmarkBlock,
                       WasmTracePhase::End, block, WasmTraceThread::Browser, generation);
    const std::uint64_t end = now();
    const std::uint64_t duration = end >= start ? end - start : 0;
    samples[block] = duration;
    result.totalUs += duration;
    if (config.deadlineUs != 0 && duration > config.deadlineUs) ++result.deadlineMisses;
  }
...
PicoTracker_Wasm_RunBenchmark(std::uint32_t iterations, std::uint32_t warmup,
  ...
  result = WasmBenchmark::Run({WasmBenchmarkConfig::Version, iterations, warmup, deadlineUs});
  return &result;
}
