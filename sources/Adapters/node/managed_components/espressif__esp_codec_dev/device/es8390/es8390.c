...
static int es8390_update_bits(audio_codec_es8390_t *codec, uint8_t reg_addr, uint8_t mask, uint8_t val)
{
    ...
    es8390_read_reg(codec, reg_addr, &regval);
    regval &= ~mask;
    regval |= (val & mask);
    return es8390_write_reg(codec, reg_addr, regval);
}

int es8390_set_bias_standby(audio_codec_es8390_t *codec)
{
...
int es8390_set_bias_on(audio_codec_es8390_t *codec)
{
...
static int es8390_set_mute(const audio_codec_if_t *h, bool mute)
{
    ...
    es8390_write_reg(codec, ES8390_ADC_SP_CONTROL_REG0x20, regv);
    return ret;
}
...
static int es8390_set_mic_gain(const audio_codec_if_t *h, float db)
{
    ...
    int ret = es8390_write_reg(codec, ES8390_PGA1_GAIN_CONTROL_REG0x72, gain_db | (3 << 4));  // MIC gain scale
    return ret == 0 ? ESP_CODEC_DEV_OK : ESP_CODEC_DEV_WRITE_FAIL;
}

static void es8390_pa_power(audio_codec_es8390_t *codec, es_pa_setting_t pa_setting)
{
...
static int es8390_config_sample(audio_codec_es8390_t *codec, int sample_rate, int bits)
{
    int ret = ESP_CODEC_DEV_OK;
    ...
    if (coeff < 0) {
        ...
        ret |= es8390_write_reg(codec, ES8390_CLK_MANAGER_REG0x04, coeff_div[coeff].Reg0x04);
        ret |= es8390_write_reg(codec, ES8390_CLK_MANAGER_REG0x05, coeff_div[coeff].Reg0x05);
        ret |= es8390_write_reg(codec, ES8390_CLK_MANAGER_REG0x06, coeff_div[coeff].Reg0x06);
        ret |= es8390_write_reg(codec, ES8390_CLK_MANAGER_REG0x07, coeff_div[coeff].Reg0x07);
        ret |= es8390_write_reg(codec, ES8390_CLK_MANAGER_REG0x08, coeff_div[coeff].Reg0x08);
        ret |= es8390_write_reg(codec, ES8390_CLK_MANAGER_REG0x09, coeff_div[coeff].Reg0x09);
        ret |= es8390_write_reg(codec, ES8390_CLK_MANAGER_REG0x0A, coeff_div[coeff].Reg0x0A);
        ret |= es8390_update_bits(codec, ES8390_CLK_MANAGER_REG0x0F, 0xC0, coeff_div[coeff].Reg0x0F);
        ret |= es8390_write_reg(codec, ES8390_CLK_MANAGER_REG0x11, coeff_div[coeff].Reg0x11);
        ret |= es8390_write_reg(codec, ES8390_ADC_SP_CONTROL_REG0x21, coeff_div[coeff].Reg0x21);
        ret |= es8390_write_reg(codec, ES8390_ADC_SP_CONTROL_REG0x22, coeff_div[coeff].Reg0x22);
        ret |= es8390_write_reg(codec, ES8390_ADC_SP_CONTROL_REG0x26, coeff_div[coeff].Reg0x26);
        ret |= es8390_update_bits(codec, 0x30, 0xC0, coeff_div[coeff].Reg0x30);
        ret |= es8390_write_reg(codec, ES8390_DAC_CONTROL_REG0x41, coeff_div[coeff].Reg0x41);
        ret |= es8390_write_reg(codec, ES8390_DAC_CONTROL_REG0x42, coeff_div[coeff].Reg0x42);
        ret |= es8390_update_bits(codec, ES8390_DAC_CONTROL_REG0x43, 0x81, coeff_div[coeff].Reg0x43);
        ret |= es8390_update_bits(codec, ES8390_CHIP_MISC_CONTROL_REG0xF0, 0x73, coeff_div[coeff].Reg0xF0);
        ret |= es8390_write_reg(codec, ES8390_CSM_STATE_REG0xF1, coeff_div[coeff].Reg0xF1);
        ret |= es8390_write_reg(codec, 0x16, coeff_div[coeff].Reg0x16);
        ret |= es8390_write_reg(codec, 0x18, coeff_div[coeff].Reg0x18);
        ret |= es8390_write_reg(codec, 0x19, coeff_div[coeff].Reg0x19);
    }
...
static int es8390_open(const audio_codec_if_t *h, void *cfg, int cfg_size)
{
    ...
    if (ES8390_Analog_DriveSel == ES8390_DriveSel_LowPower) {
        es8390_write_reg(codec, ES8390_ANALOG_CONTROL_REG0x6B, 0x80);
        es8390_write_reg(codec, ES8390_ANALOG_CONTROL_REG0x6C, 0x0F);
        es8390_write_reg(codec, ES8390_ANALOG_CONTROL_REG0x70, 0x66);
    }

    ret |= es8390_write_reg(codec, ES8390_ADC_SP_CONTROL_REG0x23, (int)(0x00 + (0 & 0xC0) + (0 << 2) + (0 & 0x03)));
    ...
    ret |= es8390_write_reg(codec, ES8390_MISC_CONTROL_REG0x01, (int)(0x08 + (0 << 7) + (0 << 6) + (0 << 5) + (0 << 0)));
    ...
    ret |= es8390_write_reg(codec, ES8390_ADC_SP_CONTROL_REG0x20, (int)(0x00 + ES8390_S16_LE + (0 << 4) + ES8390_DAIFMT_I2S + (0 << 1) + (0 << 0)));
    ret |= es8390_write_reg(codec, ES8390_DAC_CONTROL_REG0x40, (int)(0x00 + ES8390_S16_LE + (0 << 4) + ES8390_DAIFMT_I2S + (0 << 1) + (0 << 0)));
    ...
    ret |= es8390_write_reg(codec, ES8390_DAC_CONTROL_REG0x44, (int)(0x00 + (0 << 3) + (0 << 2) + (0 << 1) + (0 << 0)));
    ...
    if (codec_cfg->master_mode) {
        ...
        es8390_update_bits(codec, ES8390_MISC_CONTROL_REG0x01, ES8390_MASK_MSModeSel, 1);
    } else {
        ESP_LOGI(TAG, "Work in Slave mode");
        es8390_update_bits(codec, ES8390_MISC_CONTROL_REG0x01, ES8390_MASK_MSModeSel, 0);
    }

    // Select clock source for internal mclk
    if (codec_cfg->use_mclk) {
        es8390_update_bits(codec, ES8390_CLK_MANAGER_REG0x02, 0xC0, 0 << 6);
        codec->use_mclk = true;
    } else {
        es8390_update_bits(codec, ES8390_CLK_MANAGER_REG0x02, 0xC0, 1 << 6);
        codec->use_mclk = false;
    }
    // MCLK inverted or not
    if (codec_cfg->invert_mclk) {
        es8390_update_bits(codec, ES8390_CLK_MANAGER_REG0x02, 0x02, 1 << 1);
    } else {
        es8390_update_bits(codec, ES8390_CLK_MANAGER_REG0x02, 0x02, 0 << 1);
    }
    ...
    if (codec_cfg->invert_sclk) {
        es8390_update_bits(codec, ES8390_CLK_MANAGER_REG0x02, 0x01, 1 << 0);
    } else {
        es8390_update_bits(codec, ES8390_CLK_MANAGER_REG0x02, 0x01, 0 << 0);
    }
...
static int es8390_set_fs(const audio_codec_if_t *h, esp_codec_dev_sample_info_t *fs)
{
    ...
    if (!codec->use_mclk) {
        es8390_config_sample(codec, fs->sample_rate,fs->bits_per_sample);
    }

    es8390_set_bits_per_sample(codec, fs->bits_per_sample);
    es8390_config_fmt(codec, ES_I2S_NORMAL);
    es8390_set_bias_standby(codec);
    es8390_set_bias_on(codec);
    return ESP_CODEC_DEV_OK;
}
