...
HeapSamplePool::~HeapSamplePool() { Reset(); }

void HeapSamplePool::ReleaseSample(std::uint32_t index) {
...
std::uint32_t HeapSamplePool::GetAvailableSampleStorageSpace() {
  std::uint64_t used = 0;
...
bool HeapSamplePool::CheckSampleFits(int sampleSize) {
  return sampleSize > 0 && static_cast<std::uint32_t>(sampleSize) <=
                               GetAvailableSampleStorageSpace();
}

bool HeapSamplePool::loadSample(const char *name) {
  ...
  if (!CheckSampleFits(static_cast<int>(sampleBytes))) {
    Trace::Error("SAMPLEPOOL", "Not enough sample heap for sample: %s", name);
    wave.Close();
    return false;
  }
...
bool HeapSamplePool::unloadSample(std::uint32_t index) {
  if (index >= count_) {
    return false;
  }
  ReleaseSample(index);
