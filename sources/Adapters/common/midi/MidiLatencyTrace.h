...
#ifndef PICOTRACKER_SHARED_MIDI_LATENCY_TRACE_H
#define PICOTRACKER_SHARED_MIDI_LATENCY_TRACE_H
...
public:
  ...
  static void OutputDrained(const MidiPacket &packet, NowFunction now) noexcept;
  ...
  static void
  ResetCorrelationsForTesting(std::uint32_t inputSeed = 0U,
                              std::uint32_t outputSeed = 0U) noexcept;
};
