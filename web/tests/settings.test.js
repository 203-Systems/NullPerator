...
describe('versioned workbench settings', () => {
  ...
  it('normalizes unsafe values while retaining explicit disabled trace and mute', () => {
    expect(migrateSettings({
      ...
      version: 4, traceMask: 0, lowLatencyAudio: 1, showVirtualControls: false, developerMode: true,
    })).toMatchObject({
      displayScale: 'fit', audioBufferFrames: 8192, outputVolume: 0,
      traceMask: 0, lowLatencyAudio: true, showVirtualControls: false, developerMode: true,
    })
  })
  ...
  it('persists updates, publishes isolated snapshots, and resets atomically', () => {
    ...
    expect(JSON.parse(written.get(SETTINGS_STORAGE_KEY))).toMatchObject({
      version: 4,
      outputVolume: 42,
      showVirtualControls: false,
      developerMode: true,
