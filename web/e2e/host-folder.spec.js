...
async function runSuccessfulHostAction(page, name) {
  const region = page.getByRole('region', { name: 'Computer folder' })
  const last = region.getByText(/Last synced:/)
...
test('host-folder mirror mounts an OPFS test handle, synchronizes, and resolves a visible conflict', async ({ page }) => {
  ...
  await runSuccessfulHostAction(page, 'Sync both ways')
  ...
  await runSuccessfulHostAction(page, 'Import changes')
  ...
  await runSuccessfulHostAction(page, 'Save changes')
  ...
  await page.getByRole('button', { name: 'Sync both ways', exact: true }).click()
  ...
  // Navigating away must release the native dialog's top-layer entry. The
  // pending conflict remains in the store and is shown again when Files is
  // reopened through the same primary navigation used by normal users.
  const trackerNav = page.getByRole('button', { name: 'Tracker', exact: true })
  const filesNav = page.getByRole('button', { name: 'Files', exact: true })
  await trackerNav.evaluate((element) => element.click())
