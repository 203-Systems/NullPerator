...
test('AudioWorklet is the default and presents a user-facing sound gate', async ({ page }) => {
  ...
  try {
    ...
    const generation = await canvas.getAttribute('data-action-generation')
    await page.keyboard.press('s')
    await expect(canvas).toHaveAttribute('data-action-generation', generation)
    await page.keyboard.press('Tab')
    await expect(enableSound).toBeFocused()
    const developerToggle = page.getByRole('button', { name: 'Developer mode' })
    const toggleBox = await developerToggle.boundingBox()
    expect(toggleBox).not.toBeNull()
    await page.mouse.click(toggleBox.x + toggleBox.width / 2, toggleBox.y + toggleBox.height / 2)
    await expect.poll(() => gate.evaluate((element) => element.contains(document.activeElement))).toBe(true)
    await expect(developerToggle).not.toBeFocused()
