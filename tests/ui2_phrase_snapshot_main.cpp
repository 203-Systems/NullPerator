...
int main(int argc, char **argv) {
  if (argc != 3) {
    std::cerr << "usage: ui2_phrase_snapshot_tool "
                 "<note|empty|fx|number> <output.ppm>\n";
    return 2;
  }
  const std::string_view state = argv[1];
  if (state != "note" && state != "empty" && state != "fx" &&
      state != "number") {
    return 3;
  }
  ui2::UiPalette palette;
  ui2::UiFrameScene scene;
  const ui2::UiPhraseViewData data = ui2::test::ApprovedPhraseFixture(state);
  if (ui2::UiPhraseView::Build(data, palette, scene) !=
