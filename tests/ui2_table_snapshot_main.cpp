...
int main(int argc, char **argv) {
  if (argc != 3) {
    std::cerr << "usage: ui2_table_snapshot_tool <state> <output.ppm>\n";
    return 2;
  }
  const std::string_view state = argv[1];
  if (state != "phrase" && state != "instrument" && state != "phrase-number" &&
      state != "instrument-number") {
    return 3;
  }
  ui2::UiPalette palette;
  ui2::UiFrameScene scene;
  const ui2::UiTableViewData data = ui2::test::ApprovedTableFixture(state);
  if (ui2::UiTableView::Build(data, palette, scene) !=
