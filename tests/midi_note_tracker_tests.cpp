...
TEST_CASE("MIDI note registration honors the requested audio channel") {
  ...
  REQUIRE(tracker.registerNote(60U, 2U, 7U));
...
TEST_CASE("Repeated MIDI note-ons release one tracked voice per note-off") {
  ...
  REQUIRE(tracker.registerNote(60U, 2U, 0U));
  REQUIRE(tracker.registerNote(60U, 2U, 1U));
...
TEST_CASE("MIDI note tracking recovers after all eight voices are occupied") {
  ...
  for (std::uint8_t channel = 0U; channel < MidiNoteTracker::kVoiceCount;
       ++channel) {
  ...
  REQUIRE(tracker.registerNote(72U, 0U, 3U));
  CHECK(tracker.unregisterNote(72U, 0U) == 3);
  REQUIRE(tracker.registerNote(72U, 0U, 3U));
  CHECK(tracker.getNextAvailableChannel() == -1);
}
