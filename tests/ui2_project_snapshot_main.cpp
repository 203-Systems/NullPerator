...
int main(int argc, char **argv) {
  ...
  if (state != "name" && state != "playback" && state != "cleanup" &&
      state != "render") {
    return 3;
  }
  ui2::UiPalette palette;
  ui2::UiFrameScene scene;
  const ui2::UiProjectViewData data = ui2::test::ApprovedProjectFixture(state);
  if (ui2::UiProjectView::Build(data, palette, scene) !=
