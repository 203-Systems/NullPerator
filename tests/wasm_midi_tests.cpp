...
TEST_CASE("WASM MIDI byte queue preserves realtime bytes interleaved with "
          "channel data") {
...
TEST_CASE(
    "WASM MIDI byte queue accepts or rejects each browser batch atomically") {
...
TEST_CASE("WASM MIDI packet queue reserves realtime capacity and drains by "
          "sequence") {
...
TEST_CASE(
    "WASM MIDI output encodes channel, short, and realtime packet lengths") {
  const MidiPacket note = QueuedMidiOutDevice::Encode(
  ...
  const MidiPacket program = QueuedMidiOutDevice::Encode(
  ...
  const MidiPacket clock = QueuedMidiOutDevice::Encode(
      MidiMessage(MidiMessage::MIDI_CLOCK, 99U, 99U), 7.0);
  CHECK(clock.length == 1);
  CHECK((clock.bytes == std::array<std::uint8_t, 3>{0xF8, 0, 0}));
}

TEST_CASE("WASM MIDI input traces one accepted-to-processed latency per "
          "browser batch") {
  Profiler::Stop();
  ...
  const std::uint32_t generation =
      Profiler::Start(static_cast<std::uint32_t>(TraceCategory::Midi));
  ...
  CHECK(traced[0].name == TraceName::MidiInputAccepted);
  CHECK(traced[0].timestampUs == 10'000U);
  CHECK(traced[0].value == trace.correlation);
  CHECK(traced[0].thread == TraceThread::Browser);
  CHECK(traced[1].name == TraceName::MidiInputLatencyUs);
  CHECK(traced[1].phase == TracePhase::Counter);
  CHECK(traced[1].thread == TraceThread::Application);
...
TEST_CASE("WASM MIDI input batch crossing the 512-byte poll budget settles "
          "only once") {
  Profiler::Stop();
  MidiLatencyTrace::ResetCorrelationsForTesting();
  ...
  const MidiLatencyTrace::Ticket trace =
      MidiLatencyTrace::CaptureInput(&MidiNow);
  REQUIRE(queue.Push(bytes, 77.0, trace.timestampMilliseconds, trace.generation,
                     trace.correlation));
  MidiLatencyTrace::PublishInputAccepted(trace);
...
TEST_CASE(
    "WASM MIDI output queue latency excludes the future scheduled send delay") {
  Profiler::Stop();
  MidiLatencyTrace::ResetCorrelationsForTesting();
  midiTimeMilliseconds = 100.0;
  Profiler::Start(static_cast<std::uint32_t>(TraceCategory::Midi));

  QueuedMidiOutDevice::Queue queue;
  QueuedMidiOutDevice output(queue, &MidiNow);
  ...
  const auto traced = DrainMidiTrace();
  const auto queued =
      std::find_if(traced.begin(), traced.end(), [](const auto &record) {
        return record.name == TraceName::MidiOutputQueued;
      });
  const auto latency =
      std::find_if(traced.begin(), traced.end(), [](const auto &record) {
        return record.name == TraceName::MidiOutputLatencyUs;
      });
...
TEST_CASE(
    "WASM MIDI trace correlations wrap independently without publishing zero") {
  Profiler::Stop();
...
TEST_CASE(
    "WASM MIDI trace timestamps and latency saturate abnormal clocks safely") {
...
TEST_CASE("WASM MIDI tracing disabled reads no clock and leaves queue behavior "
          "unchanged") {
  Profiler::Stop();
  midiClockReads.store(0U, std::memory_order_relaxed);
